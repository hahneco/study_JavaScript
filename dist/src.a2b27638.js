...
// completeButton生成関数
var makeCompleteButton = function makeCompleteButton(target) {
  // 完了ボタン生成
  target.innerText = "完了";
  target.addEventListener("click", function () {
    // 完了リストに追加する要素
    var addTarget = target.parentNode;
    tmpAddTarget = addTarget;

    // TODO内テキストを取得
    var text = tmpAddTarget.firstElementChild.innerText;
...
var makeDeleteButtonButton = function makeDeleteButtonButton(target) {
  target.innerText = "削除";
  target.addEventListener("click", function () {
    // 押されたボタンの親要素を未完了リストから削すする
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "63275" + '/');
